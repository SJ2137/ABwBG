...
# Wczytanie danych genotypowych z pliku `.ped`, informacji o osobnikach z pliku `.fam` i informacji o mapowaniu z pliku `.map`.

```{r}
setwd("C:/Users/48721/Desktop/everything/studia/magisterka/semestr2/R/ABWG-GWAS")
Geno <- read_ped("sativas413.ped")
#wczytanie kolumn jako osobnych wartości
p = Geno$p
n = Geno$n
Geno = Geno$x
head(Geno)
...
Przekonwertowanie danych na macierz i jej transpozycja.
...
Podanie wymiarów macierzy (liczby osobników i markerów SNP).

```{r}
dim(Geno)
```

# Wczytanie danych fenotypowych i sprawdzenie ich zgodności z danymi genotypowymi.
...
#sprawdzanie wymiarów nowej macierzy
dim(Geno1)
```
